...
async function main(): Promise<void> {
  ...
  const db = new NeDbDatabase({
    ...
    articlesPath: config.db.articlesPath,
  });
  await db.init();
  ...
  app.use("/api/admin", adminRoutes({ db, devices, auth, credentials }));
...
/** Seed an initial admin from env when the credential store is empty. */
async function seedAdmin(store: NeDbCredentialStore): Promise<void> {
  if ((await store.count()) > 0) return;
  const { user, pass } = config.adminBootstrap;
  if (!user || !pass) {
    console.warn(
  ...
  console.log(`[auth] seeded bootstrap admin "${user}"`);
}
